...
final class SapientiaFilterManager: NSObject, ObservableObject {
  ...
  var statusText: String {
    switch status {
    case .approvalRequired:
      return "Enable Sapientia in System Settings › Network › Filters to finish setup."
    case .disabled:
    ...
    case .failed(let message):
      return message
    case .installing:
      return "macOS may ask you to approve the Sapientia network filter."
    case .notConfigured:
  ...
  private func configureFilter() {
    ...
    manager.loadFromPreferences { [weak self] error in
      ...
      configuration.organization = "Sapientia"
      configuration.vendorConfiguration = self.vendorConfiguration(
        for: self.latestRules ?? .disabled)

      manager.localizedDescription = "Sapientia Website Filter"
...
extension SapientiaFilterManager: OSSystemExtensionRequestDelegate {
  func request(
      ...
      if request === developmentResetRequest {
        ...
        @unknown default:
          finishDevelopmentReset(
            .failure(
              NSError(
                domain: "SapientiaDevelopmentReset",
                code: 1,
                userInfo: [
