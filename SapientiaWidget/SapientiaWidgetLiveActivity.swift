...
struct SapientiaWidgetAttributes: ActivityAttributes {
...
struct SapientiaWidgetLiveActivity: Widget {
  ...
  var body: some WidgetConfiguration {
    ActivityConfiguration(for: SapientiaWidgetAttributes.self) { context in
  ...
  private func sapientiaLogo(size: CGFloat) -> some View {
    Image("SapientiaStickerLogo")
      .resizable()
      .scaledToFit()
  ...
  private func expandedIslandTitle(name: String) -> some View {
    HStack(alignment: .center, spacing: 8) {
      sapientiaLogo(size: expandedLogoSize)
  ...
  private func expandedIslandStatusView(
    for state: SapientiaWidgetAttributes.ContentState
  ) -> some View {
  ...
  private func compactElapsedTimerText(for state: SapientiaWidgetAttributes.ContentState) -> some View {
    Text(
      Date(timeIntervalSinceNow: state.getTimeIntervalSinceNow()),
      style: .timer
    )
...
extension SapientiaWidgetAttributes {
  fileprivate static var preview: SapientiaWidgetAttributes {
    SapientiaWidgetAttributes(
      name: "Focus Session",
      message: "Stay focused and avoid distractions")
  }
}

extension SapientiaWidgetAttributes.ContentState {
  fileprivate static var shortTime: SapientiaWidgetAttributes.ContentState {
    SapientiaWidgetAttributes
      .ContentState(
  ...
  fileprivate static var longTime: SapientiaWidgetAttributes.ContentState {
    SapientiaWidgetAttributes.ContentState(
      startTime: Date(timeInterval: 60, since: Date.now),
  ...
  fileprivate static var breakActive: SapientiaWidgetAttributes.ContentState {
    SapientiaWidgetAttributes.ContentState(
      startTime: Date(timeInterval: 60, since: Date.now),
  ...
  fileprivate static var pauseActive: SapientiaWidgetAttributes.ContentState {
    SapientiaWidgetAttributes.ContentState(
      startTime: Date(timeInterval: 60, since: Date.now),
...
#Preview("Notification", as: .content, using: SapientiaWidgetAttributes.preview) {
  SapientiaWidgetLiveActivity()
} contentStates: {
  SapientiaWidgetAttributes.ContentState.shortTime
  SapientiaWidgetAttributes.ContentState.longTime
  SapientiaWidgetAttributes.ContentState.breakActive
  SapientiaWidgetAttributes.ContentState.pauseActive
}
