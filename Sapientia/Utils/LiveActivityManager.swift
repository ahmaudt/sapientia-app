...
class LiveActivityManager: ObservableObject {
  ...
  @Published var currentActivity: Activity<SapientiaWidgetAttributes>?

  // Use AppStorage for persisting the activity ID across app launches
  @AppStorage("com.sapientia.currentActivityId") private var storedActivityId: String = ""
  ...
  private func restoreExistingActivity() {
    ...
    if !storedActivityId.isEmpty {
      if let existingActivity = Activity<SapientiaWidgetAttributes>.activities.first(where: {
  ...
  func endSessionActivity() {
    ...
    // End the activity
    let completedState = SapientiaWidgetAttributes.ContentState(
      startTime: Date.now
    )
