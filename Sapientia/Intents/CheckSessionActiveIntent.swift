...
struct CheckSessionActiveIntent: AppIntent {
  ...
  static var title: LocalizedStringResource = "Check if Sapientia Session is Active"
  static var description = IntentDescription(
    "Check if any Sapientia blocking session is currently active and return true or false. Useful for automation and shortcuts."
  )
  ...
  func perform() async throws -> some IntentResult & ReturnsValue<Bool> & ProvidesDialog {
    ...
    let dialogMessage =
      isActive
      ? "A Sapientia session is currently active."
      : "No Sapientia session is active."
